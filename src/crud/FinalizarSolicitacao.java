...
public class FinalizarSolicitacao {
	...
	public static void finalizaSolicitacoes() throws SQLException {
		...
		// Comando sql
		String sql1 = "SELECT c.id_solicitacao id_solicitacao, " + "c.id_cliente id_cliente, " + "e.nome nome, "
				+ "e.cpf cpf ," + "ca.id_carro id_carro," + "ca.modelo modelo," + "c.dt_solicitacao dt_solicitacao,"
				+ "c.dt_retirada dt_retirada, " + "c.dt_dev_prevista dt_prevista," + "c.dt_devolucao dt_devolucao,"
				+ "c.qtd_dias_aluguel qtd_dias_aluguel," + "c.valor_diaria valor_diaria,"
				+ "c.total_diaria total_diaria," + "c.ativo ativo " + "FROM solicitacao_tb c, " + "cliente_tb e, "
				+ "carro_tb ca " + "WHERE e.id_cliente = c.id_cliente " + "AND c.id_carro = ca.id_carro "
				+ "AND c.id_cliente = ? " + "AND c.ativo = 'S' " + // Pega que estão ativas
				"ORDER BY 1 DESC;";
		...
		while (resultado.next()) {
			...
			solicitacoes.add(new Solicitacao(id_solicitacao, id_cliente, id_carro, dt_solicitacao, dt_retirada,
					dt_prevista, dt_devolucao, qtd_dias_aluguel, valor_diaria, total_diaria, ativo, nome, cpf, modelo));
		...
		for (Solicitacao s : solicitacoes) {
			...
			System.out.println("ID: " + s.getId_solicitacao() + " id_cliente: " + s.getId_cliente() + " nome "
					+ s.getNome() + " CPF " + s.getCpf() + " id_carro: " + s.getId_carro() + " modelo: " + s.getModelo()
					+ " dt_solicitacao: " + s.getDt_solicitacao() + " dt_retirada " + s.getDt_retirada()
					+ " dt_prevista " + s.getDt_prevista() + " dt_devolucao: " + s.getDt_devolucao()
					+ " qtd_dias_aluguel " + s.getQtd_dias_aluguel() + " valor_diaria R$: " + s.getValor_diaria()
					+ " total_diaria R$: " + s.getTotal_diaria() + " ativo " + s.getAtivo());
		...
		// Comando que faz update na base setando a solicitacao como inativa e gravando
		// na base a data de devolução que é o dia atual
